  ...
  useEffect(() => {
    async function load() {
      ...
      if (snap.exists()) {
        ...
        const clientSnap = await getDoc(doc(db, "users", inv.clientId));
        const cData = clientSnap.exists() ? (clientSnap.data() as any) : null;
        setClientName(cData?.displayName || cData?.email || inv.clientId);
      }
    }
    load();
  }, [params]);
