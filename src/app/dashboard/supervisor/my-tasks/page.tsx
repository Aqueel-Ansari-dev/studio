...
export default function SupervisorMyTasksPage() {
  ...
  if (isLoading || authLoading) {
    return (
      <div className="space-y-6">
        <PageHeader title="My Assigned Tasks" description="Loading your projects..." />
        <Card>
  ...
  if (!user) {
     return (
      <div className="space-y-6">
        <PageHeader title="My Assigned Tasks" description="Please log in to see your tasks." />
         <Card>
          <CardContent className="p-6 text-center text-muted-foreground">
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="My Assigned Tasks" description="Select a project to view and manage your assigned tasks." />
      
      {projects.length === 0 ? (
        <Card>
          <CardContent className="p-6 text-center text-muted-foreground">
            <Briefcase className="mx-auto h-12 w-12 mb-4" />
            <p className="font-semibold">No tasks assigned to you yet.</p>
          ...
          {projects.map((project) => (
            <Card key={project.id} className="flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
              ...
              <CardContent className="flex-grow">
                {/* Potentially add task count here in the future */}
              </CardContent>
              <CardFooter>
                <Button asChild className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">
                  <Link href={`/dashboard/supervisor/my-tasks/${project.id}`}>
                    View My Tasks <ArrowRight className="ml-2 h-4 w-4" />
                  </Link>
                </Button>
              </CardFooter>
            </Card>
